...
static int tx_h_frame_filter(struct nrc_trx_data *tx)
{
	...
	if (discard_ipv6_mc_frame && tx->vif->type == NL80211_IFTYPE_AP) {
		/* please refer to RFC 2464 section 7 */
		if (hdr->addr1[0]==0x33 && hdr->addr1[1]==0x33) {
			nrc_mac_dbg("[%s] discard IPv6 multicast Tx frame", __func__);
			return -1;
		}
	}

	if (discard_ipv4_mc_frame && tx->vif->type == NL80211_IFTYPE_AP) {
		/* please refer to RFC 1112 */
		if (hdr->addr1[0]==0x01 && hdr->addr1[1]==0x00 &&
			hdr->addr1[2]==0x5e) {
			nrc_mac_dbg("[%s] discard IPv4 multicast Tx frame", __func__);
			return -1;
		}
	}
...
static int rx_h_nrc_action(struct nrc *nw, struct sk_buff *skb)
{
	__le16 fc;
	u16 sn;
	...
	mgmt = (void *)(skb->data + nw->fwinfo.rx_head_size - sizeof(struct hif));
	fc = mgmt->frame_control;
	sn = (le16_to_cpu(mgmt->seq_ctrl) & IEEE80211_SCTL_SEQ) >> 4;
	if (ieee80211_is_action(fc)) {
		if (mgmt->u.action.category == WLAN_CATEGORY_VENDOR_SPECIFIC) {
			p = (u8*)&mgmt->u.action.category + 1;
			if (*p == 0xc0 && *(p+1) == 0xff && *(p+2) == 0xee) {
				if (debug_action_frame & 1) {
					u8 *addr = mgmt->sa;
					uint16_t *len = (uint16_t*)(p + 4);
					pr_err("[%s][SA-%02x:%02x:%02x:%02x:%02x:%02x][SN %d] type:%d len:%d\n",
						__func__, addr[0], addr[1], addr[2],
						addr[3], addr[4], addr[5], sn,
						*(p+3), *len);
				}
				return 0;
			}
...
static void nrc_transfer_raw_socket(void *data, struct net_device *dev)
{
...
int nrc_mac_rx(struct nrc *nw, struct sk_buff *skb)
{
	...
	if (rx_h_nrc_action(nw, skb) == 0) {
		struct ieee80211_mgmt *mgmt;
		mgmt = (void *)(skb->data + nw->fwinfo.rx_head_size - sizeof(struct hif));
		if(nrc_stats_sta_list(mgmt->sa)){
			nrc_add_rx_s1g_radiotap_header(nw, skb);
			ieee80211_iterate_active_netdev(nw->hw, nrc_transfer_raw_socket, skb);
			return 0;
		}
		else{
			struct sk_buff *skb_qos = NULL;
			struct ieee80211_hdr_3addr_qos *qosnullfunc;

			nrc_common_dbg("[STA Not Found] "MACSTR"\n",MAC2STR(mgmt->sa));

			//if STA is not connected, send qos null frame to AP's mac80211
			skb_qos = NULL;
			skb_qos = dev_alloc_skb(IEEE80211_MAX_FRAME_LEN);
			skb_reserve(skb_qos, sizeof(qosnullfunc));
			qosnullfunc = (struct ieee80211_hdr_3addr_qos *) skb_put(skb_qos, sizeof(*qosnullfunc));
			qosnullfunc->frame_control = cpu_to_le16(IEEE80211_FTYPE_DATA |
								IEEE80211_STYPE_QOS_NULL |
								IEEE80211_FCTL_TODS);

			ether_addr_copy(qosnullfunc->addr1, mgmt->da);
			ether_addr_copy(qosnullfunc->addr2, mgmt->sa);
			ether_addr_copy(qosnullfunc->addr3, mgmt->da);

			qosnullfunc->qc = cpu_to_le16(0);
			ieee80211_rx_irqsafe(nw->hw, skb_qos);
	...
	if (!rx.result) {
		...
		/* rearm soft_ps_timer when AP receives frames from STA with PM0 */
		if (ap_ps_time > 0 && !ieee80211_has_pm(mh->frame_control) &&
			rx.sta && rx.vif && rx.vif->type == NL80211_IFTYPE_AP) {
			struct nrc_sta *i_sta = to_i_sta(rx.sta);
			if (i_sta && i_sta->state == IEEE80211_STA_AUTHORIZED) {
				//nrc_dbg(NRC_DBG_PS, "%s: recv frame pm 0. rearm timer (%d sec) (%pM)", __func__, ap_ps_time, rx.sta->addr);
...
nrc_add_rx_s1g_radiotap_header(struct nrc *nw, struct sk_buff *skb)
{
	...
	if (ndp_ind) {
		...
		rt_hdr_agg.rt_ampdu_ref = !rx_h_nrc_action(nw, skb) ? 0 : nrc_ampdu_mon_get_ref_id(nw, skb);
