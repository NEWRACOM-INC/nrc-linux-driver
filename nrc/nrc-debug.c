...
static int nrc_debugfs_credit_print(struct seq_file *s, void *data)
{
	int val = *(int *)s->private;
	int i, b, e;
	...
	for (i = b; i < e; i++) {
		...
		nrc_hif_cspi_read_credit(g_nw->hif, i, &front, &rear, &credit);
		seq_printf(s, "%d %d %d %d\n", i, front, rear, credit);
	}

	for (i = b; i < e; i++) {
		seq_printf(s, "%d %d %d\n", i, atomic_read(&g_nw->tx_credit[i]), atomic_read(&g_nw->tx_pend[i]));
	}

	return 0;
}

static int nrc_debugfs_credit_vif0_open(struct inode *inode, struct file *file)
{
	int *d = (int *)kzalloc(sizeof(int), GFP_KERNEL);
	*d = 0;
	g_nw = (struct nrc*)inode->i_private;
	return single_open(file, nrc_debugfs_credit_print, (void*)d);
}

static const struct file_operations credit_vif0_ops = {
	.open		= nrc_debugfs_credit_vif0_open,
	.read		= seq_read,
	//.write		= nrc_debugfs_credit_vif0_write,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int nrc_debugfs_credit_vif1_open(struct inode *inode, struct file *file)
{
	int *d = (int *)kzalloc(sizeof(int), GFP_KERNEL);
	*d = 1;
	g_nw = (struct nrc*)inode->i_private;
...
static const struct file_operations credit_vif1_ops = {
	.open		= nrc_debugfs_credit_vif1_open,
	.read		= seq_read,
	//.write		= nrc_debugfs_credit_vif1_write,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
#endif
static struct dentry *credit_debugfs_root;
...
#define nrc_debugfs_create_file(name, fops) \
	...
	credit_debugfs_root = debugfs_create_dir("credit", nw->debugfs);
	debugfs_create_file("vif0", 0644, credit_debugfs_root, nw, &credit_vif0_ops);
	debugfs_create_file("vif1", 0644, credit_debugfs_root, nw, &credit_vif1_ops);
