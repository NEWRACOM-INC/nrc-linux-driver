...
struct tim_info_node {
	struct wim_tim_info ti;
	struct list_head list;
};

static struct list_head tim_list;
static int32_t tim_list_num;
...
static void _nrc_tim_send(unsigned long data)
{
	struct nrc *nw = (struct nrc *)data;
#else
static void _nrc_tim_send(struct timer_list *t)
{
	struct nrc *nw = from_timer(nw, t, tim_send_timer);
#endif
	struct sk_buff *skb;
	struct wim_tim_param *p;
	u16 l = 0, n = 0;
	int i = 0;

	if (tim_list_num == 0 && list_empty(&tim_list)) {
		//pr_err("[%s] tim_list is empty", __func__);
		mod_timer(&nw->tim_send_timer, jiffies +
				msecs_to_jiffies(nw->beacon_int * 2));
		return;
	}
	...
	if (!skb) {
		nrc_mac_dbg("[%s] Failed to allocate skb for TIM", __func__);
		mod_timer(&nw->tim_send_timer, jiffies +
				msecs_to_jiffies(nw->beacon_int * 2));
		return;
	}

	n = tim_list_num > 140 ? 140 : tim_list_num;
	l = sizeof(u8) + n * sizeof(struct wim_tim_info);
	p = nrc_wim_skb_add_tlv(skb, WIM_TLV_TIM_PARAM, l, NULL);
	memset(p, 0, l);
	p->count = n;

	for (i = 0; i < n; i++) {
		struct tim_info_node *cur = list_first_entry(&tim_list, struct tim_info_node, list);
		if (cur) {
			p->tim_info[i].aid = cur->ti.aid;
			p->tim_info[i].set = cur->ti.set;
			nrc_mac_dbg("[%s] (%d) add tlv(aid:%d set:%d)", __func__,
					i, p->tim_info[i].aid, p->tim_info[i].set);
			list_del(&cur->list);
			kfree(cur);
			tim_list_num--;
		} else {
			nrc_mac_dbg("[%s] No more TIM info to send", __func__);
			break;
		}
	}
...
void nrc_tim_list_init(struct nrc* nw)
{
	INIT_LIST_HEAD(&tim_list);
	tim_list_num = 0;
#if KERNEL_VERSION(4, 15, 0) > LINUX_VERSION_CODE
	setup_timer(&nw->tim_send_timer, _nrc_tim_send, (unsigned long)nw);
#else
	timer_setup(&nw->tim_send_timer, _nrc_tim_send, 0);
#endif
}

void nrc_tim_list_remove(struct nrc* nw)
{
	struct tim_info_node *cur, *next;

	del_timer_sync(&nw->tim_send_timer);
	tim_list_num = 0;
	if (list_empty(&tim_list) != 0) {
		nrc_mac_dbg("[%s] tim_list is empty", __func__);
		return;
	}

	list_for_each_entry_safe(cur, next, &tim_list, list) {
		nrc_mac_dbg("[%s] remove tim_info(aid:%d)", __func__, cur->ti.aid);
		list_del(&cur->list);
		kfree(cur);
	}
}
	...
	switch (nw->chip_id) {
		...
		case 0x7394:
			if (is_mesh) {
				...
				if (nw->fwinfo.chip_rev_num > 0) {
					// if metal revised 7394, uses WIM_ENCDEC_HW.
					sw_enc = WIM_ENCDEC_HW;
				} else {
					sw_enc = WIM_ENCDEC_SW;
				}
			}
...
void nrc_wake_tx_queue(struct ieee80211_hw *hw, struct ieee80211_txq *txq)
{
	...
	if (nw->drv_state == NRC_DRV_PS) {
		...
		if (!(nw->twt_sched && twt_force_sleep)) {
			nrc_hif_wake_target(nw->hif, 0);
		}
	}
...
struct sk_buff *g_rtcm_skb;
static int nrc_vendor_update_rtcm(struct ieee80211_hw *hw,
			struct ieee80211_vif *vif)
{
	...
	control.sta = NULL;
#endif
	mgmt = (struct ieee80211_mgmt*)g_rtcm_skb->data;
	memcpy(mgmt->sa, vif->addr, ETH_ALEN);
	memcpy(mgmt->bssid, vif->addr, ETH_ALEN);
	
	txi = IEEE80211_SKB_CB(g_rtcm_skb);
	txi->control.vif = vif;
#ifdef CONFIG_SUPPORT_NEW_MAC_TX
	nrc_mac_tx(hw, &control, g_rtcm_skb);
#else
	nrc_mac_tx(hw, skb);
#endif
	return 0;
				 ...
				 struct ieee80211_vif *vif)
{
	...
	if (vif->type == NL80211_IFTYPE_AP) {
		...
		/* CKLEE_TODO, check capability */
		// nrc_twt_sched_start(nw, vif);
		/* CKLEE_TODO, stop if changed to sta */
		nrc_tim_list_init(nw);
	}
				...
				struct ieee80211_vif *vif)
{
	 ...
	 */
#if 0
	nw->hw->conf.flags &= ~IEEE80211_CONF_IDLE;
	nrc_mac_config(nw->hw, IEEE80211_CONF_CHANGE_IDLE);
#else
	if(nw->drv_state == NRC_DRV_PS) {
		dev_info(nw->dev, "Wake target for remove interface");
		ret = nrc_ps_set_mode(nw, NRC_PS_NONE, 2000);
		if (ret == -1) {
			dev_err(nw->dev, "Failed to wake to remove interface\n");
		}
	}
#endif
	...
	for (i = 0; i < ARRAY_SIZE(nw->vif); i++) {
		if (nw->vif[i]) {
			if (nw->vif[i]->type == NL80211_IFTYPE_STATION) {
				...
				ap_max_idle_timer_stop (nw, to_i_vif(nw->vif[i])); /* when restart or WDT , remove_interface is not called */
				nrc_free_vif_index(nw, nw->vif[i]);
...
static int nrc_mac_config(struct ieee80211_hw *hw, u32 changed)
{
	...
	if (changed & IEEE80211_CONF_CHANGE_PS) {
		...
		if (nw->drv_state == NRC_DRV_PS || nw->ps_modem_enabled) {
			...
			if (nw->ps_enabled)
				...
				nrc_ps_dbg("Wake up target.\n");
				nrc_hif_wake_target(nw->hif, 2000);
			}
	...
	if (idle_mode &&
		...
		nrc_mac_dbg("%s: changed: IEEE80211_CONF_CHANGE_IDLE\n", __FUNCTION__);
		if (hw->conf.flags & IEEE80211_CONF_IDLE) {
			dev_info(nw->dev, "Changing to IDLE\n");
			nrc_ps_set_mode(nw, NRC_PS_DEEPSLEEP_NONTIM, -1);
		} else {
			dev_info(nw->dev, "Changing to Active\n");
			nrc_ps_set_mode(nw, NRC_PS_NONE, 2000);
		}
	}
...
#endif
{
	...
	if (changed & BSS_CHANGED_BEACON_INT ||
		 ...
		 */
		u16 bi, short_bi = 0;
		nw->dtim_period = info->dtim_period;
		...
		nrc_wim_skb_add_tlv(skb, WIM_TLV_DTIM_PERIOD,
				sizeof(u8), &nw->dtim_period);

		if (!timer_pending(&nw->tim_send_timer)) {
			mod_timer(&nw->tim_send_timer, jiffies +
				msecs_to_jiffies(nw->beacon_int * 2));
		}
...
#ifdef CONFIG_SUPPORT_PS_HANDLE_AP
	if (vif->type == NL80211_IFTYPE_AP &&sta) {
		if (i_sta) {
			//delete timer when disconnected
			del_timer_sync(&i_sta->ps_sta.soft_ps_timer);
			i_sta->ps_sta.in_ps = false;
			nrc_dbg(NRC_DBG_STATE, "%s: remove STA's PS timer (%pM)", __func__, sta->addr);
		}
	}
#endif
...
{
	struct tx_ba_session *ba_session = container_of(work, struct tx_ba_session, ba_session_work);
	struct nrc_sta *i_sta = ba_session->sta;
	struct ieee80211_sta *peer_sta = to_ieee80211_sta(i_sta);
	int ret = 0;

	if (!peer_sta) {
		nrc_mac_dbg("Fail to set up BA. Fail to find peer (%pM)", peer_sta->addr);
		return;
	}
	...
	switch (ba_session->state)
	{
		...
		case IEEE80211_BA_CLOSE:
			...
			i_sta->nw->ampdu_supported = true;
			i_sta->nw->ampdu_reject = false;
			if((ret = ieee80211_start_tx_ba_session(peer_sta, ba_session->tid, 0)) != 0) {
				if (ret == -EBUSY) {
					nrc_dbg(NRC_DBG_STATE, "%s: receiver does not want A-MPDU so disable BA session (TID:%d)", __func__, ba_session->tid);
					ba_session->state = IEEE80211_BA_DISABLE;
				}
				if (ret == -EAGAIN) {
					nrc_dbg(NRC_DBG_STATE, "%s: session is not idle (TID:%d)", __func__, ba_session->tid);
					ieee80211_stop_tx_ba_session(peer_sta, ba_session->tid);
		...
		case IEEE80211_BA_REJECT:
			if(jiffies_to_msecs(jiffies - ba_session->ba_req_last_jiffies) > 5000) {
				ba_session->state = IEEE80211_BA_NONE;
				ba_session->ba_req_last_jiffies = 0;
				nrc_dbg(NRC_DBG_STATE, "%s: reset ba status(TID:%d)", __func__, ba_session->tid);
			}
			break;
		default:
			break;
	...
	if (sta) {
		...
		if (i_sta) {
			for (i=0 ; i < NRC_MAX_TID; i++) {
				...
				i_sta->tx_ba_session[i].state = IEEE80211_BA_NONE;
				i_sta->tx_ba_session[i].ba_req_last_jiffies = 0;
				INIT_WORK(&i_sta->tx_ba_session[i].ba_session_work, nrc_tx_ba_session_work);
...
static void nrc_deinit_sta_ba_session(struct nrc_sta *i_sta)
{
	...
	for (i = 0; i < NRC_MAX_TID; i++) {
		if (i_sta->tx_ba_session[i].ba_session_work.func) {
			cancel_work_sync(&i_sta->tx_ba_session[i].ba_session_work);
		}
	}
}

#ifdef CONFIG_SUPPORT_PS_HANDLE_AP
static void nrc_mac_rx_null(struct nrc_sta *i_sta, bool pm)
{
	...
	u16 fc = IEEE80211_FTYPE_DATA | IEEE80211_STYPE_NULLFUNC | IEEE80211_FCTL_TODS;
	...
	/* addr1 = AP (RA), addr2 = STA (SA), addr3 = BSSID (AP) */
	ether_addr_copy(hdr->addr1, i_sta-> vif->addr);
	ether_addr_copy(hdr->addr2, sta->addr);
	ether_addr_copy(hdr->addr3, i_sta-> vif->addr);
...
static void soft_ps_timer_fn(struct timer_list *t)
{
	struct ps_status *ps = from_timer(ps, t, soft_ps_timer);
	struct nrc_sta *i_sta = container_of(ps, struct nrc_sta, ps_sta);

	if (ap_ps_time > 0 && !ps->in_ps) {
		struct ieee80211_sta *sta = to_ieee80211_sta(i_sta);
		if (sta) {
			nrc_dbg(NRC_DBG_STATE, "%s: expire PS time (last:%d). Now STA in PS (%pM)",
				__func__, jiffies_to_msecs(jiffies - i_sta->ps_sta.last_rx_jiffies), sta->addr);
			//make and send NULL PM 1 to mac80211
			nrc_mac_rx_null(i_sta, true);
			ps->in_ps = true;
		}
	}
}
...
#ifdef CONFIG_SUPPORT_PS_HANDLE_AP
			if (ap_ps_time > 0 && vif->type == NL80211_IFTYPE_AP && sta) {
				struct nrc_sta *i_sta = to_i_sta(sta);
				if (i_sta) {
					//make and start timer when STA is connected
					timer_setup(&i_sta->ps_sta.soft_ps_timer, soft_ps_timer_fn, 0);
					i_sta->ps_sta.last_rx_jiffies = jiffies;
					i_sta->ps_sta.in_ps = false;
					mod_timer(&i_sta->ps_sta.soft_ps_timer, jiffies + msecs_to_jiffies(1000 * ap_ps_time));
					nrc_dbg(NRC_DBG_STATE, "%s: start STA's PS timer (%d sec)  (%pM)", __func__, ap_ps_time, sta->addr);
				}
			}
#endif
...
(old_state == IEEE80211_STA_##old && new_state == IEEE80211_STA_##new)
	...
	if (state_changed(NOTEXIST, NONE)) {
		...
		*/
		...
		if (vif->type == NL80211_IFTYPE_AP) {
			i_sta->max_idle.idle_period = i_sta->max_idle.sta_idle_timer = i_vif->max_idle_period;
		}
			   ...
			   bool set)
{
	...
	if (!nrc_mac_is_s1g(nw)) {
		...
		struct tim_info_node *tn;

		tn = (struct tim_info_node*)kzalloc(sizeof(struct tim_info_node), GFP_KERNEL);
		if (!tn) {
			pr_err("[%s] fail to alloc\n", __func__);
			return -ENOMEM;
		}
		tn->ti.aid = sta->aid;
		tn->ti.set = set;
		list_add_tail(&tn->list, &tim_list);
		++tim_list_num;
		nrc_mac_dbg("[%s] (%d) aid:%d set:%d", __func__,
				tim_list_num, tn->ti.aid, tn->ti.set);
	}
...
void scan_complete(struct ieee80211_hw *hw, bool aborted)
{
	...
	if (idle_mode && hw->conf.flags & IEEE80211_CONF_IDLE) {
		dev_info(nw->dev, "Changing to IDLE after scan\n");
		nrc_ps_set_mode(nw, NRC_PS_DEEPSLEEP_NONTIM, -1);
	}
}
							...
							struct ieee80211_vif *vif)
{
	...
	if(nw->drv_state == NRC_DRV_PS) {
		dev_info(nw->dev, "Wake target for cancelling scan");
		ret = nrc_ps_set_mode(nw, NRC_PS_NONE, 2000);
		if (ret == -1) {
			dev_err(nw->dev, "Failed to wake to cancel scan\n");
		}
	}
		  		...
		  		struct ieee80211_scan_ies *ies)
{
	...
	nrc_mac_dbg("%s called", __FUNCTION__);
	...
	if(nw->drv_state == NRC_DRV_PS) {
		dev_info(nw->dev, "Wake target for scan\n");
		ret = nrc_ps_set_mode(nw, NRC_PS_NONE, 2000);
		if (ret == -1) {
			dev_err(nw->dev, "Failed to wake to scan\n");
			ret = -EBUSY;
			goto out_idle;
		}
	}
	...
	if(nw->drv_state != NRC_DRV_RUNNING) {
		nrc_mac_dbg("%s Not running state", __func__);
		return -EBUSY;
	}
...
out_idle:
	if (idle_mode && (hw->conf.flags & IEEE80211_CONF_IDLE)) {
		dev_info(nw->dev, "Changing to IDLE after scan fail\n");
		nrc_ps_set_mode(nw, NRC_PS_DEEPSLEEP_NONTIM, -1);
	}
						...
						struct ieee80211_key_conf *key)
{
	...
	if (ret < 0) {
		dev_err(nw->dev, "Failed to install key in HW");
		ret = 1; /* fallback to software crypto */
	}

	if (0xDEAD == ret) {
		dev_err(nw->dev, "Failed to tx EAPOL M4");
		ieee80211_hw_set(hw, SW_CRYPTO_CONTROL); /* Disable fallback to SW */
		return -EINVAL;
	}
...
#endif
{
	uint32_t tput = 1670; // SGI, 1Mhz, MCS7, 50%
...
#ifdef CONFIG_USE_KERNEL_S1G_TWT
#if 0 // To use this feature, some S1G capabilities patch is needed,
	.add_twt_setup = nrc_mac_add_twt_setup,
	.twt_teardown_request = nrc_mac_twt_teardown_request,
#endif
#endif
};
...
static u8* nrc_vendor_remove(struct nrc *nw, u8 subcmd)
{
	...
	if ((subcmd >= NRC_SUBCMD_ANNOUNCE1 && subcmd <= NRC_SUBCMD_BCAST_FOTA_4) ||
		...
		(subcmd >= NRC_SUBCMD_SOLUM_HMG0 && subcmd <= NRC_SUBCMD_SOLUM_HMG2) ||
		(subcmd == NRC_SUBCMD_REMOTECMD) ||
		(subcmd == NRC_SUBCMD_WOWLAN_PATTERN)) {
		vendor_skb_ptr = &nw->vendor_skb_beacon;
	} else if (subcmd >= NRC_SUBCMD_ANNOUNCE8 && subcmd <= NRC_SUBCMD_ANNOUNCE12) {
		vendor_skb_ptr = &nw->vendor_skb_probe_req;
	} else if (subcmd >= NRC_SUBCMD_ANNOUNCE13 && subcmd <= NRC_SUBCMD_ANNOUNCE17) {
		vendor_skb_ptr = &nw->vendor_skb_probe_rsp;
	} else if (subcmd >= NRC_SUBCMD_ANNOUNCE18 && subcmd <= NRC_SUBCMD_ANNOUNCE22) {
		vendor_skb_ptr = &nw->vendor_skb_assoc_req;
	} else if (subcmd >= NRC_SUBCMD_RTCM0 && subcmd <= NRC_SUBCMD_RTCM6) {
		vendor_skb_ptr = &g_rtcm_skb;
	}
...
#define RTCM_MAX_FRAG (246)
static u8 g_rtcm_frag_num = 0;
void rtcm_cal_frag_num(const u8 *data)
{
	uint32_t total_length = 0;

	total_length |= (*(data+4));
	total_length |= (*(data+3))<<8;
	total_length |= (*(data+2))<<16;
	total_length |= (*(data+1))<<24;
	//nrc_mac_dbg("%s: rtcm total length %d", __func__, total_length);

	g_rtcm_frag_num = (total_length + RTCM_MAX_FRAG + 4)/(RTCM_MAX_FRAG + 5);
	//nrc_mac_dbg("%s: rtcm frag num %d", __func__, g_rtcm_frag_num);
}

bool rtcm_frag_complete(u8 subcmd)
{
	return (((subcmd - NRC_SUBCMD_RTCM0)+1) == g_rtcm_frag_num);
}
				...
				const u8 *data, int data_len)
{
	...
	if ((subcmd >= NRC_SUBCMD_ANNOUNCE1 && subcmd <= NRC_SUBCMD_BCAST_FOTA_4) ||
		...
		struct ieee80211_mgmt *mgmt;
		u8 da[ETH_ALEN] = {0x1, 0xAB, 0x2, 0xCD, 0x3, 0xEF};
		u8 *p;

		if (subcmd == NRC_SUBCMD_RTCM0) {
			g_rtcm_skb = dev_alloc_skb(IEEE80211_MAX_FRAME_LEN);
			if (!g_rtcm_skb)
				return -1;

			skb_reserve(g_rtcm_skb, 16);
			mgmt = skb_put_zero(g_rtcm_skb, 24);
			memcpy(mgmt->da, da, ETH_ALEN);
			mgmt->frame_control = cpu_to_le16(IEEE80211_FTYPE_MGMT | IEEE80211_STYPE_ACTION);
		
			p = skb_put(g_rtcm_skb, 4);
			mgmt->u.action.category = WLAN_CATEGORY_VENDOR_SPECIFIC;
			*(p + 1) = (VENDOR_OUI & 0xFF0000) >> 16;
			*(p + 2) = (VENDOR_OUI & 0xFF00) >> 8;
			*(p + 3) = (VENDOR_OUI & 0xFF);

			//print_hex_dump(KERN_DEBUG, "rtcm0: ", DUMP_PREFIX_NONE,
			//		16, 1, data, data_len, false);

			rtcm_cal_frag_num(data);
		}

		if ((subcmd - NRC_SUBCMD_RTCM0 + 1) > g_rtcm_frag_num) {
			nrc_mac_dbg("%s: garbage rtcm fragment dropped.",__func__);
			return -1;
		}

		p = skb_put(g_rtcm_skb, new_elem_len);
		*p++ = WLAN_EID_VENDOR_SPECIFIC;
		*p++ = data_len + OUI_LEN + 1;
		*p++ = (VENDOR_OUI & 0xFF0000) >> 16;
		*p++ = (VENDOR_OUI & 0xFF00) >> 8;
		*p++ = (VENDOR_OUI & 0xFF);
		*p++ = subcmd;
...
static int nrc_vcmd_backup_reinstall(u8 vif_id, struct nrc *nw)
{
	...
	if ((hw == NULL) || (vif == NULL)) {
		nrc_mac_dbg("%s, No hw(0x%x) and vif(0x%x) info\n",__func__, hw, vif);
		return -EINVAL;
	}

	if (list_empty(&vcmd_backup_info[vif_id].head) != 0) {
		nrc_mac_dbg("%s, No vcmd entry\n",__func__);
		return 0;
	}

	list_for_each_entry_safe(cur, next, &vcmd_backup_info[vif_id].head, list) {
		/* Update local vendor data */
		if (nrc_vendor_update(nw, cur->subcmd, cur->data, cur->data_len) != 0) {
			nrc_mac_dbg("%s, sub_cmd:%d, error: invalid arg\n",__func__, cur->subcmd );
			continue;
		}

		if ((cur->subcmd >= NRC_SUBCMD_ANNOUNCE1 && cur->subcmd <= NRC_SUBCMD_BCAST_FOTA_4) ||
			(cur->subcmd == NRC_SUBCMD_UTC) ||
			(cur->subcmd >= NRC_SUBCMD_SOLUM_HMG0 && cur->subcmd <= NRC_SUBCMD_SOLUM_HMG2) ||
			(cur->subcmd == NRC_SUBCMD_REMOTECMD) ||
			(cur->subcmd == NRC_SUBCMD_WOWLAN_PATTERN))
			nrc_vendor_update_beacon(hw, vif);
		else if ((cur->subcmd >= NRC_SUBCMD_ANNOUNCE8) && (cur->subcmd < NRC_SUBCMD_ANNOUNCE13))
			nrc_vendor_update_probe_req(hw, vif);
		else if ((cur->subcmd >= NRC_SUBCMD_ANNOUNCE13) && (cur->subcmd < NRC_SUBCMD_ANNOUNCE18))
			nrc_vendor_update_probe_rsp(hw, vif);
		else if ((cur->subcmd >= NRC_SUBCMD_ANNOUNCE18) && (cur->subcmd <= NRC_SUBCMD_ANNOUNCE22))
			nrc_vendor_update_assoc_req(hw, vif);
		else {
			nrc_mac_dbg("%s, sub_cmd:%d, error: invalid subcmd\n",__func__, cur->subcmd );
			continue;
		}
		nrc_mac_dbg("%s, reinstalled sub_cmd:%d\n",__func__, cur->subcmd );
	}
	return 0;
}
...
static void nrc_vcmd_backup_delayed_reinstall(struct work_struct *work)
{
	...
	vcmd_info = container_of(work, VCMD_BACKUP_INFO, vcmd_backup_reinstall.work);
...
static void nrc_vcmd_backup_init_info(u8 vif_id, struct nrc *nw)
{
	if (!vcmd_backup_info[vif_id].initialized) {
		...
		INIT_DELAYED_WORK(&vcmd_backup_info[vif_id].vcmd_backup_reinstall,
											nrc_vcmd_backup_delayed_reinstall);
		nrc_mac_dbg("%s, Init vif(%d) vcmd backup info\n",__func__, vif_id);
	} else {
		if (vcmd_backup_info[vif_id].reset_by_wdt) {
			if (nw->vif[vif_id]->type == NL80211_IFTYPE_AP) {
				nrc_mac_dbg("%s,AP:vif(%d) is reset by wdt. Restore vcmds\n",__func__, vif_id);
				nrc_vcmd_backup_reinstall(vif_id, nw);

			} else if(nw->vif[vif_id]->type == NL80211_IFTYPE_STATION) {
				nrc_mac_dbg("%s,STA:vif(%d) is reset by wdt. Restore vcmds\n",__func__, vif_id);
				queue_delayed_work(nw->workqueue, &vcmd_backup_info[vif_id].vcmd_backup_reinstall,
									msecs_to_jiffies(VCMD_BACKUP_RESOTRE_WORK_DELAY_MS));
			}
			vcmd_backup_info[vif_id].reset_by_wdt = false;
		}
	}
...
void nrc_vcmd_backup_set_wdt_flag(u8 vif_id)
{
	if (vif_id < VIF_MAX) {
		...
		nrc_mac_dbg("%s, vif(%d) is reset by wdt\n",__func__, vif_id);
	}
}
...
static void _nrc_vcmd_backup_del_entry(u8 vif_id, u8 subcmd)
{
	...
	list_for_each_entry_safe(cur, next, &vcmd_backup_info[vif_id].head, list) {
		if (cur->subcmd == subcmd) {
			nrc_mac_dbg("%s, Del vcmd_entry: sub_cmd:%d\n",__func__, subcmd);
			list_del(&cur->list);
			kfree(cur);
			return;
		}
...
static int _nrc_vcmd_backup_add_entry(u8 vif_id, u8 subcmd, const u8 *data, int data_len)
{
...
static int nrc_vcmd_backup_remove_entry(struct ieee80211_vif *vif, u8 subcmd)
{
	if (((subcmd >= NRC_SUBCMD_ANNOUNCE1) && (subcmd <= NRC_SUBCMD_ANNOUNCE7)) ||
		((subcmd >= NRC_SUBCMD_ANNOUNCE8) && (subcmd <= NRC_SUBCMD_ANNOUNCE22)) ||
		((subcmd >= NRC_SUBCMD_SOLUM_HMG0) && (subcmd <= NRC_SUBCMD_SOLUM_HMG2))) {
...
static int nrc_vcmd_backup_add_entry(struct ieee80211_vif *vif, u8 subcmd, const u8 * data, int data_len)
{
	if (((subcmd >= NRC_SUBCMD_ANNOUNCE1) && (subcmd <= NRC_SUBCMD_ANNOUNCE7)) ||
		((subcmd >= NRC_SUBCMD_ANNOUNCE8) && (subcmd <= NRC_SUBCMD_ANNOUNCE22)) ||
		((subcmd >= NRC_SUBCMD_SOLUM_HMG0) && (subcmd <= NRC_SUBCMD_SOLUM_HMG2))) {
				...
				u8 subcmd, const void *data, int data_len)
{
	...
	if ((subcmd < NRC_SUBCMD_BCAST_FOTA_1) ||
		...
		(subcmd == NRC_SUBCMD_REMOTECMD) ||
		(subcmd == NRC_SUBCMD_WOWLAN_PATTERN)) {
		g_nrc_beacon_updated = 0;
		return nrc_vendor_update_beacon(hw, vif);
	} else if ((subcmd >= NRC_SUBCMD_BCAST_FOTA_1) && (subcmd < NRC_SUBCMD_ANNOUNCE8)) {
		return 0;
	} else if ((subcmd >= NRC_SUBCMD_ANNOUNCE8) && (subcmd < NRC_SUBCMD_ANNOUNCE13)) {
		return nrc_vendor_update_probe_req(hw, vif);
	} else if ((subcmd >= NRC_SUBCMD_ANNOUNCE13) && (subcmd < NRC_SUBCMD_ANNOUNCE18)) {
		return nrc_vendor_update_probe_rsp(hw, vif);
	} else if ((subcmd >= NRC_SUBCMD_ANNOUNCE18) && (subcmd <= NRC_SUBCMD_ANNOUNCE22)) {
		return nrc_vendor_update_assoc_req(hw, vif);
	} else if ((subcmd >= NRC_SUBCMD_RTCM0) && (subcmd <= NRC_SUBCMD_RTCM6)) {
		if (rtcm_frag_complete(subcmd)) {
			nrc_mac_dbg("%s: Send rtcm action frame", __func__);
			return nrc_vendor_update_rtcm(hw, vif);
		} else {
			return 0;
		}
	} else {
...
static int nrc_vendor_cmd_announce21(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_ANNOUNCE21,
				 data, data_len);
}

static int nrc_vendor_cmd_announce22(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_ANNOUNCE22,
				 data, data_len);
}

static int nrc_vendor_cmd_utc(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_UTC,
				 data, data_len);
}

static int nrc_vendor_cmd_solum_hmg0(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_SOLUM_HMG0,
				 data, data_len);
}

static int nrc_vendor_cmd_solum_hmg1(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_SOLUM_HMG1,
				 data, data_len);
}

static int nrc_vendor_cmd_solum_hmg2(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_SOLUM_HMG2,
				 data, data_len);
}

static int nrc_vendor_cmd_rtcm0(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_RTCM0,
				 data, data_len);
}

static int nrc_vendor_cmd_rtcm1(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_RTCM1,
				 data, data_len);
}

static int nrc_vendor_cmd_rtcm2(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_RTCM2,
				 data, data_len);
}

static int nrc_vendor_cmd_rtcm3(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_RTCM3,
				 data, data_len);
}

static int nrc_vendor_cmd_rtcm4(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_RTCM4,
				 data, data_len);
}

static int nrc_vendor_cmd_rtcm5(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_RTCM5,
				 data, data_len);
}

static int nrc_vendor_cmd_rtcm6(struct wiphy *wiphy,
				struct wireless_dev *wdev,
				const void *data, int data_len)
{
	return nrc_vendor_cmd_append(wiphy, wdev, NRC_SUBCMD_RTCM6,
				 data, data_len);
}
...
static struct wiphy_vendor_command nrc_vendor_cmds[] = {
	...
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_ANNOUNCE21
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_announce21,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_ANNOUNCE22
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_announce22,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	...
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_SOLUM_HMG0
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_solum_hmg0,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_SOLUM_HMG1
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_solum_hmg1,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_SOLUM_HMG2
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_solum_hmg2,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_RTCM0
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_rtcm0,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_RTCM1
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_rtcm1,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_RTCM2
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_rtcm2,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_RTCM3
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_rtcm3,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_RTCM4
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_rtcm4,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_RTCM5
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_rtcm5,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
	},
	{
		.info = {
			.vendor_id = OUI_IEEE_REGISTRATION_AUTHORITY,
			.subcmd = NRC_SUBCMD_RTCM6
		},
		.flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = nrc_vendor_cmd_rtcm6,
#if KERNEL_VERSION(5, 3, 0) <= NRC_TARGET_KERNEL_VERSION
		.policy = VENDOR_CMD_RAW_DATA,
		.maxattr = MAX_VENDOR_ATTR,
#endif
...
void nrc_bcn_mon_timer(struct timer_list *t)
{
	...
	if (disable_cqm_on_scan) {
		...
		if(nw->scan_mode == NRC_SCAN_MODE_IDLE) {
			...
			nrc_mac_dbg("In scan state, delay beacon loss event\n");
...
/*	char tmp[2];*/
	...
	if (ret < 0) {
		dev_err(nw->dev, "ieee80211_register_hw failed (%d)", ret);
...
void nrc_send_beacon_loss(struct nrc *nw)
{
	...
	dev_info(nw->dev, "beacon loss event to vif(%d)", i_vif->index);
