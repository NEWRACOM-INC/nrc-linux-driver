...
static void spi_show_status(struct spi_device *spi)
{
	...
	pr_info("SPI Status:");
	pr_info("  TX_SLOT count: %u, head: %u, tail: %u size: %u",
			priv->slot[TX_SLOT].count,
			priv->slot[TX_SLOT].head,
			priv->slot[TX_SLOT].tail,
			priv->slot[TX_SLOT].size);
	pr_info("  RX_SLOT count: %u, head: %u, tail: %u size: %u",
			priv->slot[RX_SLOT].count,
	...
	for (i = 0; i < CREDIT_QUEUE_MAX; i++) {
		...
		nrc_hif_cspi_read_credit(hdev, i, &front, &rear, &credit);
		pr_info("CREDIT[%d] front=%d rear=%d credit=%d\n",
				i, front, rear, credit);
	}

	for (i = 0; i < IEEE80211_NUM_ACS*3; i++) {
		pr_info(KERN_INFO "AC[%d] tx_credit=%d tx_pend=%d\n",
				i,
				atomic_read(&nw->tx_credit[i]),
				atomic_read(&nw->tx_pend[i]));
	}
	pr_info("-------------------------------------------------");
	pr_info("SPI IRQ Mask: 0x%x", priv->irq_enabled_mask);
}

static int spi_ack_failure(struct spi_device *spi) {
	ssize_t status;
	u8 tx[8];
	u8 rx[8];
	struct spi_transfer xfer[2];
	struct nrc *nw = spi_get_drvdata(spi);
	struct nrc_hif_device *hdev = NULL;
	...
	dev_err(&spi->dev, "[%s]", __func__);

	spi_show_status(spi);

	memset(tx, 0xff, sizeof(tx));
	spi_set_transfer(&xfer[0], tx, NULL, 8); /* hwFSM init */
	spi_set_transfer(&xfer[1], tx, rx, 8);   /* hwFSM confirming */
	status = spi_sync_transfer(spi, xfer, 2);

	if (rx[7] != C_SPI_ACK) {
		dev_err(&spi->dev, "[%s] spi ack is invalid (0x%02x)", __func__, rx[7]);
	}
		...
		u8 addr, u8 *buf, ssize_t size)
{
	...
	if (size > 1) {
		...
		spi_set_transfer(&xfer[5], &dummy, NULL, sizeof(dummy));
	} else {
		spi_set_transfer(&xfer[1], &dummy, NULL, sizeof(dummy));
	...
	arr_len = (size > 1) ? ARRAY_SIZE(xfer) : 4;
	...
	if (WARN_ON_ONCE(rx[7] != C_SPI_ACK)) {
		dev_err(&spi->dev, "[%s] SPI ACK is invalid (0x%02X)\n", __func__, rx[7]);
		print_hex_dump(KERN_ERR, "spi_resp ", DUMP_PREFIX_NONE, 16, 1, rx, 8, false);
		spi_ack_failure(spi);
		return -EIO;
	}
#endif
...
static int _c_spi_write_reg(struct spi_device *spi, u8 addr, u8 data)
{
	...
	status = spi_sync_transfer(spi, xfer, 4);
	if (status < 0) {
				...
				struct nrc_spi_priv *priv)
{
	...
	skb = dev_alloc_skb(priv->slot[RX_SLOT].size * priv->max_slot_num);
	if (!skb)
		goto fail;

	if (c_spi_num_slots(priv, RX_SLOT) == 0) {
		ret = spi_update_status(priv->spi);
		if (ret < 0) {
			dev_err(nw->dev, "%s: spi update failed", __func__);
			goto fail;
		}
	}
	...
	if (c_spi_num_slots(priv, RX_SLOT) < nr_slot) {
		ret = spi_update_status(priv->spi);
		if (ret < 0) {
			dev_err(nw->dev, "%s: spi update failed (rx_slot:%d, nr_slot:%d)", __func__,
...
int g_nrc_beacon_updated = 0;
int nrc_get_beacon_updated(void)
{
	return g_nrc_beacon_updated;
}
static int spi_update_status(struct spi_device *spi)
{
	struct nrc *nw = spi_get_drvdata(spi);
	...
	if (priv->hw.sys.chip_id == 0x7394) {
		if (nw->drv_state == NRC_DRV_PS) {
			c_spi_enable_irq(spi, false, CSPI_EIRQ_A_ENABLE); /* cleanup shadow reg */
	...
	SYNC_UNLOCK(hdev);
	if (ret < 0 || status->eirq.status > EIRQ_STATUS_DEVICE_SLEEP) {
		nrc_dbg(NRC_DBG_COMMON, "spi_update_status error drv_state:%d status:0x%02x mode:0x%02x enable:0x%02x 0x%08X",
				nw->drv_state, status->eirq.status, status->eirq.mode, status->eirq.enable, status->msg[3]);
		return ret;
	}
	...
	if (status->eirq.status & EIRQ_STATUS_DEVICE_READY) {
		...
		switch (status->msg[3] & 0xffff) {
			...
			case TARGET_NOTI_BEACON_UPDATED:
				g_nrc_beacon_updated++;
				//nrc_dbg(NRC_DBG_STATE, "TARGET_NOTI_BEACON_UPDATED");
				break;

			case TARGET_NOTI_WDT_EXPIRED:
				if (nw->drv_state == NRC_DRV_PS) {
					/**
					 * EXCEPTION: WDT reset happens while processing deep sleep operation.
					 * Then, it's necessary to recover HSPI and disconnect first.
					 */
			...
			case TARGET_NOTI_TWT_QUIET:
				nrc_dbg(NRC_DBG_STATE, "TARGET_NOTI_TWT_QUIET");
				twt_service = false;
				sysfs_notify(&mod->mkobj.kobj, NULL, "twt_service");
				break;
			default:
				nrc_dbg(NRC_DBG_COMMON, "spi_update_status msg error drv_state:%d status:0x%02x mode:0x%02x enable:0x%02x 0x%08X",
						nw->drv_state, status->eirq.status, status->eirq.mode, status->eirq.enable, status->msg[3]);
				return ret;
		}
...
update:
	...
	if (c_spi_num_slots(priv, RX_SLOT) > 33) {
		...
		if (priv->hw.sys.chip_id == 0x7394 && nw->drv_state == NRC_DRV_PS &&
			((status->msg[3] & 0xffff) == TARGET_NOTI_REQUEST_FW_DOWNLOAD)) {
			//(7394) to avoid calling spi_reset_rx() that makes fail to get status for FW download
...
static int spi_wait_for_xmit(struct nrc_hif_device *hdev, struct sk_buff *skb)
{
	...
	ret = spi_update_status(priv->spi);
	if (ret < 0) {
		nrc_mac_dbg("%s: spi update status failed\n", __func__);
		return ret;
	}

	if (c_spi_num_slots(priv, TX_SLOT) < priv->max_slot_num) {
...
static int spi_suspend(struct nrc_hif_device *dev)
{
	...
	if (nw->drv_state == NRC_DRV_RUNNING &&
		(nw->wowlan_enabled || (power_save >= NRC_PS_DEEPSLEEP_TIM) || idle_mode)) {
...
void spi_reset_rx (struct nrc_hif_device *hdev)
{
	...
	dev_err(&spi->dev, "Reset SPI RX head:%u tail:%u\n", priv->slot[RX_SLOT].head, priv->slot[RX_SLOT].tail);
	priv->slot[RX_SLOT].tail = priv->slot[RX_SLOT].head = 0;
...
void spi_reset_tx (struct nrc_hif_device *hdev)
{
	...
	spi_disable_irq(hdev);
	c_spi_enable_irq(spi, false, CSPI_EIRQ_A_ENABLE);
	dev_err(&spi->dev, "Reset SPI TX head:%u tail:%u\n", priv->slot[TX_SLOT].head, priv->slot[TX_SLOT].tail);
...
static void spi_config_fw(struct nrc_hif_device *dev)
{
	...
	trace_nrc_hif_rx_slot(priv, RX_SLOT, "config");
	trace_nrc_hif_tx_slot(priv, TX_SLOT, "config");
...
static void spi_update(struct nrc_hif_device *hdev)
{
	...
	trace_nrc_hif_rx_slot(priv, RX_SLOT, "spi update");
	trace_nrc_hif_tx_slot(priv, TX_SLOT, "spi update");
...
static bool spi_check_ready(struct nrc_hif_device *hdev)
{
...
#if 0 // for debug
	nrc_dbg(NRC_DBG_HIF, "[%s] chip_id:%04x build_chip_id:%04x target_sw_id:%u host_sw_id:%u  status:%d",
	...
	dev_info(&spi->dev, "FW is ready (chip_id:%04x build_chip_id:%04x target_sw_id:%u host_sw_id:%u  status:%d)",
				sys.chip_id, (sys.sw_id >> 16), sys.sw_id & 0xFFFF, NRC_SW_ID, sys.status);
...
static void c_spi_config(struct nrc_spi_priv *priv)
{
	...
	priv->max_slot_num = DIV_ROUND_UP(sizeof(struct hif) + ETH_DATA_LEN + 32, priv->slot[RX_SLOT].size);

	dev_info(&priv->spi->dev,
		"Newracom IEEE802.11 C-SPI: chipid=%04x, sw_id=%04x, board_id=%04X  fw_name=%s bd_name=%s",
