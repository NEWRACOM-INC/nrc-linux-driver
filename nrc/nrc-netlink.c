...
#define DBG_PRINT 1
#define LED_HEAD_SIZE (4)  // max_aid(2) + led_data_len(2)

// 3-bit LED/WAKE system configuration
#define BITS_PER_AID       3
#define AID_DATA_MASK      0x07
#define LED_COLOR_MASK     0x03  // bit[1:0] = LED color  
#define WAKE_FLAG_MASK     0x04  // bit[2] = WAKE flag
#define AID_PATTERN_CYCLE  8    // 8 AID pattern repeats every 3 bytes (8*3=24 bits)
#define PATTERN_BYTES      3    // 3 bytes per pattern cycle (24 bits)

// Debug output configuration
// LED Debug Configuration
#define DEBUG_LED_KERNEL_LOGS 0  // 0=disable, 1=enable LED debug logs in kernel
#define DEBUG_AIDS_PER_LINE  20  // Number of AIDs to display per debug line

/* LED/WAKE netlink command structure (must match userspace) */
struct nrc_led_netlink_cmd {
	u16  max_aid;            // Maximum AID in the network
	u16 led_data_len;       // Length of LED data in bytes  
	u8  led_data[0];        // LED+WAKE data (3-bit per AID: 2-bit LED + 1-bit WAKE)
} __packed;

/* 8-AID pattern extraction structure (same as userspace) */
struct aid_extract_pattern {
	u8 byte_offset;     // 그룹 내 바이트 위치 (0, 1, 2)
	u8 bit_pos;         // 바이트 내 시작 비트 위치  
	bool spans_boundary; // 바이트 경계 넘나듦 여부
	u8 bits_in_first;   // 첫 바이트에서 추출할 비트 수
	u8 bits_in_second;  // 둘째 바이트에서 추출할 비트 수
};

/* 8-AID 추출 패턴 룩업 테이블 (userspace와 동일) */
static const struct aid_extract_pattern aid_extract_patterns[AID_PATTERN_CYCLE] = {
	{0, 0, false, 3, 0},  // AID 1: byte[0] bit[0:2]
	{0, 3, false, 3, 0},  // AID 2: byte[0] bit[3:5] 
	{0, 6, true,  2, 1},  // AID 3: byte[0] bit[6:7] + byte[1] bit[0]
	{1, 1, false, 3, 0},  // AID 4: byte[1] bit[1:3]
	{1, 4, false, 3, 0},  // AID 5: byte[1] bit[4:6]
	{1, 7, true,  1, 2},  // AID 6: byte[1] bit[7] + byte[2] bit[0:1]
	{2, 2, false, 3, 0},  // AID 7: byte[2] bit[2:4]
	{2, 5, false, 3, 0},  // AID 8: byte[2] bit[5:7]
};

#if DEBUG_LED_KERNEL_LOGS
/**
 * extract_3bit_aid_data_optimized - Extract 3-bit LED+WAKE data using 8-AID pattern
 * @led_data: Packed LED data array
 * @aid: AID number (1-based)
 * @data_len: Length of led_data array
 * 
 * Returns: 3-bit data (bit[1:0]=LED color, bit[2]=WAKE flag) or 0 if invalid
 */
static u8 extract_3bit_aid_data_optimized(const u8 *led_data, int aid, int data_len)
{
	int pattern_idx = (aid - 1) % AID_PATTERN_CYCLE;
	int group = (aid - 1) / AID_PATTERN_CYCLE;
	const struct aid_extract_pattern *p = &aid_extract_patterns[pattern_idx];
	int group_base = group * PATTERN_BYTES;
	u8 result = 0;
	
	// Check if we have enough data for this group
	if (group_base + p->byte_offset >= data_len)
		return 0;
	
	if (!p->spans_boundary) {
		// All 3 bits in one byte - simple extraction
		result = (led_data[group_base + p->byte_offset] >> p->bit_pos) & AID_DATA_MASK;
	} else {
		// Bits span across byte boundary - use pre-calculated split info
		u8 first_mask = (1 << p->bits_in_first) - 1;
		u8 second_mask = (1 << p->bits_in_second) - 1;
		
		result = (led_data[group_base + p->byte_offset] >> p->bit_pos) & first_mask;
		
		if (group_base + p->byte_offset + 1 < data_len) {
			result |= (led_data[group_base + p->byte_offset + 1] & second_mask) << p->bits_in_first;
		}
	}
	
	return result & AID_DATA_MASK;
}
#endif

/* nrc_netlink_led_cmd - LED command handler for netlink interface
 *
 * Handles LED control commands via WIM_CMD_LED_EVENT
 */
static int nrc_netlink_led_cmd(struct sk_buff *skb, struct genl_info *info)
{
	struct sk_buff *wim_skb;
	struct nrc_led_netlink_cmd *led_cmd = NULL;
	int data_len;
	...
	if (data_len < LED_HEAD_SIZE) {
		nrc_common_dbg("LED command too small: %d bytes", data_len);
		return -EINVAL;
	}

	// Check if total size matches header + actual data
#if ENABLE_BADVA_FIX_READ
	{
		u16 max_aid = get_unaligned((u16 *)&led_cmd->max_aid);
		u16 led_data_len = get_unaligned((u16 *)&led_cmd->led_data_len);

		if (data_len != (LED_HEAD_SIZE + led_data_len)) {
			nrc_common_dbg("LED command size mismatch: received=%d, expected=%d",
				       data_len, LED_HEAD_SIZE + led_data_len);
			return -EINVAL;
		}

		nrc_common_dbg("LED command received: max_aid=%d, led_data_len=%d, total_len=%d",
			       max_aid, led_data_len, data_len);
	}
#else
	if (data_len != (LED_HEAD_SIZE + led_cmd->led_data_len)) {
		nrc_common_dbg("LED command size mismatch: received=%d, expected=%d", 
			       data_len, LED_HEAD_SIZE + led_cmd->led_data_len);
		return -EINVAL;
	}

	nrc_common_dbg("LED command received: max_aid=%d, led_data_len=%d, total_len=%d",
		       led_cmd->max_aid, led_cmd->led_data_len, data_len);
#endif

#if DEBUG_LED_KERNEL_LOGS
	// Debug: Show LED+WAKE states for each AID (3-bit system)
	{
		char line_buf[256];  // Increased buffer for LED+WAKE display (RW format)
		int aid, line_pos;

#if ENABLE_BADVA_FIX_READ
		u16 max_aid_debug = get_unaligned((u16 *)&led_cmd->max_aid);
		u16 led_data_len_debug = get_unaligned((u16 *)&led_cmd->led_data_len);

		for (aid = 1; aid <= max_aid_debug; aid += DEBUG_AIDS_PER_LINE) {
			int end_aid = (aid + DEBUG_AIDS_PER_LINE - 1 <= max_aid_debug) ?
				      aid + DEBUG_AIDS_PER_LINE - 1 : max_aid_debug;
#else
		for (aid = 1; aid <= led_cmd->max_aid; aid += DEBUG_AIDS_PER_LINE) {
			int end_aid = (aid + DEBUG_AIDS_PER_LINE - 1 <= led_cmd->max_aid) ? 
				      aid + DEBUG_AIDS_PER_LINE - 1 : led_cmd->max_aid;
#endif
			int i;

			line_pos = snprintf(line_buf, sizeof(line_buf), "AID %d-%d: ", aid, end_aid);

			for (i = aid; i <= end_aid; i++) {
#if ENABLE_BADVA_FIX_READ
				u8 aid_data = extract_3bit_aid_data_optimized(led_cmd->led_data, i, led_data_len_debug);
#else
				u8 aid_data = extract_3bit_aid_data_optimized(led_cmd->led_data, i, led_cmd->led_data_len);
#endif
				u8 led_color = aid_data & LED_COLOR_MASK;
				u8 wake_flag = (aid_data & WAKE_FLAG_MASK) ? 1 : 0;
				const char *color_str;

				color_str = (led_color == 0) ? "X" : 
					    (led_color == 1) ? "R" :
					    (led_color == 2) ? "G" : "B";

				// Display format: ColorWake (e.g., R1, G0, X0)
				line_pos += snprintf(line_buf + line_pos, sizeof(line_buf) - line_pos, 
						     "%s%d ", color_str, wake_flag);

				if (line_pos >= sizeof(line_buf) - 20) break;
			}

			nrc_common_dbg("%s", line_buf);
		}
	}
#endif	

	wim_skb = nrc_wim_alloc_skb(nrc_nw, WIM_CMD_LED_EVENT, 2048);
	if (!wim_skb)
		return -ENOMEM;

#if ENABLE_BADVA_FIX_READ
	{
		u16 led_data_len_tlv = get_unaligned((u16 *)&led_cmd->led_data_len);
		nrc_wim_skb_add_tlv(wim_skb, WIM_TLV_LED_DATA, led_data_len_tlv, led_cmd->led_data);
	}
#else
	nrc_wim_skb_add_tlv(wim_skb, WIM_TLV_LED_DATA, led_cmd->led_data_len, led_cmd->led_data);
#endif
	nrc_xmit_wim_request(nrc_nw, wim_skb);
