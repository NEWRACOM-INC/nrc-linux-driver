...
{
#if ENABLE_BADVA_FIX_WRITE
	put_unaligned(t, &tlv->t);
	put_unaligned(l, &tlv->l);
#else
	tlv->t = t;
	tlv->l = l;
#endif
...
u64 nrc_wim_get_tsf (struct nrc *nw, struct ieee80211_vif *vif)
{
	...
	if (wim->cmd == WIM_CMD_GET) {
		...
		u16 tlv_type = get_unaligned(&tlv->t);
		if (tlv_type == WIM_TLV_TSF) {
			u16 tlv_len = get_unaligned(&tlv->l);
			memcpy(&tsf, &tlv->v, tlv_len);
#else
...
#if ENABLE_BADVA_FIX_READ
		tlv_type = get_unaligned(&tlv->t);
		if (tlv_type == WIM_TLV_MACADDR_PARAM) {
#else
...
int nrc_wim_rx(struct nrc *nw, struct sk_buff *skb, u8 subtype)
{
	...
	if (subtype >= HIF_WIM_SUB_MAX) {
		pr_err("Invalid WIM subtype (%d)", subtype);
		dev_kfree_skb(skb);
		return 0;
	}
	if (!wim_rx_handler[subtype]) {
		pr_err("No WIM handler (%d)", subtype);
		dev_kfree_skb(skb);
		return 0;
	}
