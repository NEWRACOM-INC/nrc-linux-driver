...
#if ENABLE_BADVA_FIX_READ
	tlv_type = get_unaligned(&sleep_duration->h.type);
	if (wim->cmd == WIM_CMD_SLEEP && tlv_type == WIM_TLV_SLEEP_DURATION) {
#else
		...
		struct sk_buff *skb, int timeout)
{
	...
	if (cmd == resp_wim->cmd) {
		...
		if (tlv_type == WIM_TLV_RETURN) {
			u16 tlv_len = get_unaligned(&tlv->l);
			memcpy(&ret, &tlv->v, tlv_len);
		}
#else
