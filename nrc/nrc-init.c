 ...
 */
int ampdu_mode = NRC_AMPDU_DISABLE;
...
/**
 * discard IPv6 Multicast frame for Solum 8K AP
 */
//bool discard_ipv6_mc_frame = false;
bool discard_ipv6_mc_frame = true;
module_param(discard_ipv6_mc_frame , bool, 0600);
MODULE_PARM_DESC(discard_ipv6_mc_frame , "discard IPv6 Multicast frame for Solum 8K AP");

/**
 * discard IPv6 Multicast frame for Solum 8K AP
 */
//bool discard_ipv4_mc_frame = false;
bool discard_ipv4_mc_frame = true;
module_param(discard_ipv4_mc_frame , bool, 0600);
MODULE_PARM_DESC(discard_ipv4_mc_frame , "discard IPv4 Multicast frame for Solum 8K AP");
...
/**
 * Action frame debugging
 */
int debug_action_frame = false;
module_param(debug_action_frame, int, 0600);
MODULE_PARM_DESC(debug_action_frame, "Show debugging info for action frames");
...
int ap_ps_time = 1; //in second
module_param(ap_ps_time, int, 0600);
MODULE_PARM_DESC(ap_ps_time, "Set PS timer for STA (on AP) (default: 120 seconds)");
#endif
...
static void nrc_on_fw_ready(struct sk_buff *skb, struct nrc *nw)
{
	...
	nrc_dbg(NRC_DBG_HIF, "  -- type: %d", get_unaligned(&ready->h.type));
	nrc_dbg(NRC_DBG_HIF, "  -- length: %d", get_unaligned(&ready->h.len));
#else
